...
public class VentasDAO {
   // static final DecimalFormat DFORMAT=new DecimalFormat("0.00");
    public static ImprimirObject getDocumento(int idVenta) {
        ...
        String sql = "SELECT v.NumeroSerie,v.FechaVentas,v.Monto,"
                + "e.User,e.Nombres from ventas v INNER JOIN empleado e \n"
...
//                object.setDocClient(rs.getString("c.Dni"));
    ...
    public static List<Object[]> getVentas(String fecInicio, String fecFinal) {
        List<Object[]> objectList = new ArrayList<>();
        String sql = "SELECT v.IdVentas,v.FechaVentas,v.NumeroSerie,v.Monto,e.Nombres \n"
    ...
    public int GuardarVentas(Ventas v) {
        int r = 0;
        String sql = "insert into Ventas(IdEmpleado,NumeroSerie,FechaVentas,Monto)values(?,?,?,?)";
        try {
            Connection con = Conexion.ConectarDB();
            PreparedStatement ps = con.prepareStatement(sql);
